...
import { Plus, Search, Edit, Phone, Mail, MapPin, Loader2, X, Save, Users } from 'lucide-react';
...
const Customers = () => {
  ...
  const segments = ['regular', 'member', 'vip', 'reseller', 'wholesale'];

  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-amber-100">Customers</h1>
          <p className="text-gray-400">Manage customer database</p>
        </div>
      ...
      <div className="flex gap-4">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Search by name or phone..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2 bg-[#1a1214] border border-red-900/30 rounded-lg" />
        </div>
            ...
            <div key={customer.id} className="bg-[#1a1214] border border-red-900/30 rounded-xl p-4 hover:border-red-700/50 transition-colors" data-testid={`customer-card-${customer.id}`}>
              <div className="flex justify-between items-start mb-3">
                <div>
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded capitalize ${
                    customer.segment === 'vip' ? 'bg-amber-900/30 text-amber-400' :
                    customer.segment === 'member' ? 'bg-blue-900/30 text-blue-400' :
                    customer.segment === 'reseller' ? 'bg-purple-900/30 text-purple-400' :
              ...
              <div className="mt-4 pt-3 border-t border-red-900/30 flex justify-between text-sm">
                <div>
                  ...
                  <div className="font-semibold text-amber-400">Rp {customer.total_spent?.toLocaleString() || 0}</div>
                </div>
                <div className="text-right">
                  <div className="text-gray-400">Points</div>
                  <div className="font-semibold">{customer.loyalty_points || 0}</div>
                </div>
              </div>
            </div>
          ))
