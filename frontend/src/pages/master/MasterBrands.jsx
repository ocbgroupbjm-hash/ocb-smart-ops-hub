...
const MasterBrands = () => {
  ...
  const handleDelete = async (item) => { 
    ...
    try { 
      await api(`/api/master/brands/${item.id}`, { method: 'DELETE' }); 
  ...
  return (
    <div className="space-y-4" data-testid="master-brands-page">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-amber-100">Merk</h1>
          <p className="text-gray-400 text-sm">Kelola merk produk</p>
        </div>
      </div>
      ...
      <div className="bg-[#1a1214] border border-red-900/30 rounded-xl p-4">
        <div className="relative max-w-md">
          ...
          <input type="text" placeholder="Cari merk..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} 
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="bg-[#1a1214] border border-red-900/30 rounded-xl w-full max-w-md">
            <div className="p-4 border-b border-red-900/30 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-amber-100">{editingItem ? 'Edit' : 'Tambah'} Merk</h2>
