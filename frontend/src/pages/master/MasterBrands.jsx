...
const MasterBrands = () => {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api(`/api/master/brands?search=${searchTerm}`);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const url = editingItem ? `/api/master/brands/${editingItem.id}` : '/api/master/brands';
  ...
  const handleDelete = async (item) => { if (!confirm(`Hapus "${item.name}"?`)) return; try { await api(`/api/master/brands/${item.id}`, { method: 'DELETE' }); toast.success('Berhasil dihapus'); loadData(); } catch { toast.error('Gagal menghapus'); } };

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-amber-100">Merk</h1><p className="text-gray-400 text-sm">Kelola merk produk</p></div>
