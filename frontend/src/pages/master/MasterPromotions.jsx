...
import { Plus, Search, Edit2, Trash2, Loader2, X, Gift, Calendar, Tag } from 'lucide-react';
...
const MasterPromotions = () => {
  ...
  const [formData, setFormData] = useState({ 
    code: '', name: '', promo_type: 'buy_get', description: '',
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api(`/api/master/promotions?search=${searchTerm}`);
  ...
  const handleEdit = (item) => { 
    ...
    setFormData({ 
      code: item.code || '', 
      name: item.name || '', 
      promo_type: item.promo_type || 'buy_get',
  ...
  const resetForm = () => {
    setEditingItem(null);
    setFormData({ code: '', name: '', promo_type: 'buy_get', description: '',
      start_date: '', end_date: '', is_active: true });
  };
  ...
  return (
    <div className="space-y-4" data-testid="promotions-page">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-amber-100">Periode Promosi</h1>
          <p className="text-gray-400 text-sm">Kelola program promosi</p>
        </div>
      ...
      <div className="bg-[#1a1214] border border-red-900/30 rounded-xl p-4">
        <div className="relative max-w-md">
          ...
          <input type="text" placeholder="Cari promosi..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} 
                ...
                <tr key={item.id} className="hover:bg-red-900/10">
                  ...
                  <td className="px-4 py-3 text-center">
                    <span className="px-2 py-1 bg-purple-600/20 text-purple-400 rounded text-xs">
                      {promoTypeLabels[item.promo_type] || item.promo_type}
                    </span>
                  </td>
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="bg-[#1a1214] border border-red-900/30 rounded-xl w-full max-w-lg">
            <div className="p-4 border-b border-red-900/30 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-amber-100">{editingItem ? 'Edit' : 'Tambah'} Promosi</h2>
            ...
            <form onSubmit={handleSubmit} className="p-4 space-y-4">
              ...
              <div>
                <label className="block text-sm text-gray-400 mb-1">Jenis Promosi</label>
                <select value={formData.promo_type} onChange={(e) => setFormData({ ...formData, promo_type: e.target.value })}
                  className="w-full px-3 py-2 bg-[#0a0608] border border-red-900/30 rounded-lg">
                  <option value="buy_get">Beli X Gratis Y</option>
                  <option value="bundle">Paket Bundle</option>
                  <option value="discount">Diskon Produk</option>
              ...
              <div>
                ...
                <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} 
                  ...
                  placeholder="Contoh: Beli 2 Gratis 1 untuk semua produk kategori XYZ"/>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
