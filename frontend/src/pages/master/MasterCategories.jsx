...
const MasterCategories = () => {
  ...
  const handleDelete = async (item) => {
    ...
    try {
      ...
      if (res.ok) { toast.success('Berhasil dihapus'); loadData(); }
    } catch (err) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { setEditingItem(null); setFormData({ code: '', name: '', description: '' }); setShowModal(true); }}
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="bg-[#1a1214] border border-red-900/30 rounded-xl w-full max-w-md">
            <div className="p-4 border-b border-red-900/30 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-amber-100">{editingItem ? 'Edit Kategori' : 'Tambah Kategori'}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-4 space-y-4">
              ...
              <div>
                ...
                <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                  className="w-full px-3 py-2 bg-[#0a0608] border border-red-900/30 rounded-lg" rows={3} />
              </div>
              <div className="flex justify-end gap-3 pt-4">
                ...
                <button type="submit" className="px-4 py-2 bg-gradient-to-r from-red-600 to-amber-600 text-white rounded-lg">{editingItem ? 'Update' : 'Simpan'}</button>
              </div>
            </form>
          </div>
        </div>
