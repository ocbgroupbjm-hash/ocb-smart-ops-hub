...
import { Plus, Search, Edit2, Trash2, Loader2, X, Truck, MapPin } from 'lucide-react';
import { toast } from 'sonner';

const MasterShippingCosts = () => {
  const { api } = useAuth();
  ...
  const [formData, setFormData] = useState({ 
    code: '', name: '', destination: '', cost: 0, min_weight: 0, max_weight: 0, is_active: true
  });

  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api(`/api/master/shipping-costs?search=${searchTerm}`);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const url = editingItem ? `/api/master/shipping-costs/${editingItem.id}` : '/api/master/shipping-costs';
  ...
  const handleDelete = async (item) => { 
    if (!confirm(`Hapus ongkir "${item.name}"?`)) return; 
    try { 
      await api(`/api/master/shipping-costs/${item.id}`, { method: 'DELETE' }); 
  ...
  const resetForm = () => {
    setEditingItem(null);
    setFormData({ code: '', name: '', destination: '', cost: 0, min_weight: 0, max_weight: 0, is_active: true });
  };

  return (
    <div className="space-y-4" data-testid="shipping-costs-page">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-amber-100">Ongkos Kirim</h1>
          <p className="text-gray-400 text-sm">Kelola tarif pengiriman</p>
        </div>
        <button onClick={() => { resetForm(); setShowModal(true); }} 
          className="px-4 py-2 bg-gradient-to-r from-red-600 to-amber-600 text-white rounded-lg flex items-center gap-2">
          <Plus className="h-4 w-4" /> Tambah Ongkir
        </button>
      </div>
      ...
      <div className="bg-[#1a1214] border border-red-900/30 rounded-xl p-4">
        <div className="relative max-w-md">
          ...
          <input type="text" placeholder="Cari ongkir..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} 
          ...
          <table className="w-full">
            <thead className="bg-red-900/20">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold text-amber-200">TUJUAN</th>
                <th className="px-4 py-3 text-right text-xs font-semibold text-amber-200">TARIF</th>
                <th className="px-4 py-3 text-center text-xs font-semibold text-amber-200">BERAT (KG)</th>
            ...
            <tbody className="divide-y divide-red-900/20">
              {loading ? (
                ...
                <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-400">Belum ada data ongkir</td></tr>
              ) : items.map(item => (
                <tr key={item.id} className="hover:bg-red-900/10">
                  ...
                  <td className="px-4 py-3 text-sm text-right text-green-400">Rp {(item.cost || 0).toLocaleString('id-ID')}</td>
                  <td className="px-4 py-3 text-sm text-center text-gray-300">
                    {item.min_weight || 0} - {item.max_weight || 0}
                  </td>
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="bg-[#1a1214] border border-red-900/30 rounded-xl w-full max-w-md">
            <div className="p-4 border-b border-red-900/30 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-amber-100">{editingItem ? 'Edit' : 'Tambah'} Ongkir</h2>
            ...
            <form onSubmit={handleSubmit} className="p-4 space-y-4">
              ...
              <div>
                ...
                <input type="text" value={formData.destination} onChange={(e) => setFormData({ ...formData, destination: e.target.value })} 
                  placeholder="Contoh: Jakarta, Surabaya, Dalam Kota" className="w-full px-3 py-2 bg-[#0a0608] border border-red-900/30 rounded-lg" />
              </div>
              <div>
                <label className="block text-sm text-gray-400 mb-1">Tarif (Rp)</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-gray-400 mb-1">Berat Min (Kg)</label>
                  <input type="number" step="0.1" min="0" value={formData.min_weight} 
                    onChange={(e) => setFormData({ ...formData, min_weight: Number(e.target.value) })} 
                    className="w-full px-3 py-2 bg-[#0a0608] border border-red-900/30 rounded-lg" />
                </div>
                <div>
                  <label className="block text-sm text-gray-400 mb-1">Berat Maks (Kg)</label>
                  <input type="number" step="0.1" min="0" value={formData.max_weight} 
...
export default MasterShippingCosts;
