...
import { Plus, Search, Edit2, Trash2, Loader2, X, Phone, Target } from 'lucide-react';
import { toast } from 'sonner';

const MasterSalesPersons = () => {
  const { api } = useAuth();
  ...
  const [formData, setFormData] = useState({
    code: '', name: '', phone: '', email: '', branch_id: '',
    commission_percent: 0, target_monthly: 0, is_active: true
  });
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [res, branchRes] = await Promise.all([
        api(`/api/master/sales-persons?search=${searchTerm}`),
        api('/api/branches')
      ]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const url = editingItem ? `/api/master/sales-persons/${editingItem.id}` : '/api/master/sales-persons';
  ...
  const handleEdit = (item) => { 
    ...
    setFormData({
      ...
      email: item.email || '', branch_id: item.branch_id || '',
      commission_percent: item.commission_percent || 0, target_monthly: item.target_monthly || 0,
  ...
  const handleDelete = async (item) => { 
    if (!confirm(`Hapus sales "${item.name}"?`)) return; 
    try { await api(`/api/master/sales-persons/${item.id}`, { method: 'DELETE' }); toast.success('Berhasil dihapus'); loadData(); } 
    catch { toast.error('Gagal menghapus'); } 
  };

  const resetForm = () => {
    setEditingItem(null);
    setFormData({ code: '', name: '', phone: '', email: '', branch_id: '', commission_percent: 0, target_monthly: 0, is_active: true });
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-amber-100">Daftar Sales</h1>
          <p className="text-gray-400 text-sm">Kelola data salesperson</p>
        </div>
      ...
      <div className="bg-[#1a1214] border border-red-900/30 rounded-xl p-4">
        <div className="relative max-w-md">
          ...
          <input type="text" placeholder="Cari sales..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} 
        ...
        <table className="w-full">
          <thead className="bg-red-900/20">
            <tr>
              ...
              <th className="px-4 py-3 text-right text-xs font-semibold text-amber-200">KOMISI %</th>
              <th className="px-4 py-3 text-right text-xs font-semibold text-amber-200">TARGET</th>
          ...
          <tbody className="divide-y divide-red-900/20">
            {loading ? (
              ...
              <tr><td colSpan={8} className="px-4 py-8 text-center text-gray-400">Belum ada data sales</td></tr>
            ) : items.map(item => (
              <tr key={item.id} className="hover:bg-red-900/10">
                ...
                <td className="px-4 py-3 text-sm text-right text-green-400">{item.commission_percent || 0}%</td>
                <td className="px-4 py-3 text-sm text-right text-gray-300">
                  <div className="flex items-center justify-end gap-1">
                    <Target className="h-3 w-3" /> Rp {(item.target_monthly || 0).toLocaleString('id-ID')}
                  </div>
                </td>
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="bg-[#1a1214] border border-red-900/30 rounded-xl w-full max-w-lg">
            <div className="p-4 border-b border-red-900/30 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-amber-100">{editingItem ? 'Edit' : 'Tambah'} Sales</h2>
            ...
            <form onSubmit={handleSubmit} className="p-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-gray-400 mb-1">Komisi (%)</label>
                  <input type="number" step="0.1" value={formData.commission_percent} onChange={(e) => setFormData({ ...formData, commission_percent: Number(e.target.value) })} 
                    className="w-full px-3 py-2 bg-[#0a0608] border border-red-900/30 rounded-lg" />
                </div>
                <div>
                  <label className="block text-sm text-gray-400 mb-1">Target Bulanan</label>
                  <input type="number" value={formData.target_monthly} onChange={(e) => setFormData({ ...formData, target_monthly: Number(e.target.value) })} 
                    className="w-full px-3 py-2 bg-[#0a0608] border border-red-900/30 rounded-lg" />
                </div>
              </div>
...
export default MasterSalesPersons;
