...
const MasterCustomers = () => {
  ...
  const handleDelete = async (item) => { 
    ...
    try { await api(`/api/customers/${item.id}`, { method: 'DELETE' }); toast.success('Berhasil dihapus'); loadData(); } 
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-red-900/20">
              {loading ? (
                ...
                <tr><td colSpan={8} className="px-4 py-8 text-center text-gray-400">Belum ada data pelanggan</td></tr>
              ) : items.map(item => (
