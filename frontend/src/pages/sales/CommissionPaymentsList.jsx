...
import { 
  Plus, Search, Eye, Printer, Trash2, Loader2, RefreshCw, BadgeDollarSign
} from 'lucide-react';

const formatRupiah = (num) => `Rp ${(num || 0).toLocaleString('id-ID')}`;
const formatDate = (date) => date ? new Date(date).toLocaleDateString('id-ID') : '-';

const CommissionPaymentsList = () => {
  const { api } = useAuth();
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api('/api/sales/commission-payments');
    ...
    return payments.filter(p => {
      if (filters.keyword) {
        ...
        if (!p.payment_number?.toLowerCase().includes(kw) && 
            !p.sales_name?.toLowerCase().includes(kw)) return false;
      }
      return true;
    });
  }, [payments, filters]);
  ...
  return (
    <div className="space-y-4 p-4">
      <div className="flex justify-between items-center">
        ...
        <a href="/sales/commission-payments/add" className="px-4 py-2 bg-blue-600 hover:bg-blue-700 rounded flex items-center gap-2 text-sm">
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-800">
              <tr className="text-left text-gray-400 text-xs">
                ...
                <th className="p-2 border-b border-gray-700">Kode Sales</th>
            ...
            <tbody className="divide-y divide-gray-700/50">
              {loading ? (
                ...
                <tr key={p.id} className="hover:bg-gray-800/30">
                  ...
                  <td className="p-2 font-mono text-xs">{p.sales_code || '-'}</td>
                  <td className="p-2">{p.sales_name}</td>
                  <td className="p-2 text-xs">{p.account_name || '-'}</td>
                  <td className="p-2 text-xs">{p.notes || '-'}</td>
                  <td className="p-2 text-right font-medium text-green-400">{formatRupiah(p.amount)}</td>
      ...
      <div className="bg-gray-800/50 rounded-lg border border-gray-700 p-3">
        <div className="grid grid-cols-2 gap-4 text-sm text-center">
          <div>
            <p className="text-gray-400 text-xs">Jumlah Pembayaran</p>
            <p className="font-bold text-lg">{summary.count}</p>
          </div>
          <div>
            <p className="text-gray-400 text-xs">Total Komisi Dibayar</p>
...
export default CommissionPaymentsList;
