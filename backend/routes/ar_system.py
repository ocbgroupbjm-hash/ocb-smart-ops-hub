...
DEFAULT_AR_ACCOUNTS = {
    ...
    "bad_debt_expense": {"code": "6301", "name": "Beban Piutang Tak Tertagih"}
}
...
async def create_ar_payment_journal(
    ...
    """Create journal entry for AR payment"""
    user_id = user.get("user_id") or user.get("id")
    ...
    if payment.get("payment_method") == "bank":
        debit_account = account_map.get("bank_account", DEFAULT_AR_ACCOUNTS["bank_account"])
    else:
        debit_account = account_map.get("cash_account", DEFAULT_AR_ACCOUNTS["cash_account"])
    
    credit_account = account_map.get("ar_account", DEFAULT_AR_ACCOUNTS["ar_account"])
